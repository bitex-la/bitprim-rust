...
extern "C" {
    pub fn executor_construct_fd(path: *const c_char, out_fd: c_int, err_fd: c_int) -> ExecutorP;
...
impl Executor {
        ...
        E: AsRawFd,
    {
        ...
        let raw = unsafe { executor_construct_fd(path.as_ptr(), out.as_raw_fd(), err.as_raw_fd()) };
        Executor {
            raw,
    ...
    pub fn version() -> String {
        unsafe {
            ...
            if s.is_null() {
                panic!("executor_version was null");
            }
    ...
    extern "C" fn run_handler<H>(_raw: ExecutorP, raw_context: *mut c_void, error: ExitCode)
    where
        H: FnOnce(Executor, ExitCode),
    {
        unsafe {
