...
impl Explorer {
  pub fn address_history(&self, address: PaymentAddress, limit: u64, since: u64) 
  ...
  pub fn address_incoming(&self, address: PaymentAddress, limit: u64, since: u64)
    -> Result<Vec<Received>>
  { 
    self.chain_and_history(address, limit, since).map(|(c, history)|{
      history
  ...
  pub fn address_unspents(&self, address: PaymentAddress, limit: u64, since: u64)
    -> Result<Vec<Received>>
  { 
    self.chain_and_history(address, limit, since).map(|(c, history)|{
      history
...
impl Sent {
  pub fn new(point: Point) -> Self {
    Self{ transaction_hash: point.hash().to_hex(), position: point.index() }
  }
}
...
impl AddressHistory {
  pub fn from_compact(source: &HistoryCompact, chain: &Chain) -> AddressHistory {
    ...
    match source.point_kind() {
      PointKind::Input => AddressHistory::Received(
        Received::new(source, chain.is_spent(source.point().to_output_point()))
      ),
